...
{
	var funcs, uarg, rv, next, trackTime;
	...
	rv = {
		...
		if (trackTime) {
			op['hrtimeStarted'] = null;
			op['hrtimeElapsed'] = null;
		}
	...
	if (trackTime) {
		rv['hrtimeStarted'] = process.hrtime();
		rv['hrtimeElapsed'] = null;
	}

	if (funcs.length === 0) {
		setImmediate(function () {
			if (trackTime) {
				mod_assert.ok(rv['hrtimeStarted']);
				rv['hrtimeElapsed'] = process.hrtime(
				    rv['hrtimeStarted']);
			}
	...
	next = function (err, result) {
		...
		if (trackTime) {
			mod_assert.ok(entry['hrtimeStarted']);
			entry['hrtimeElapsed'] = process.hrtime(
			    entry['hrtimeStarted']);
		}
	...
	if (trackTime) {
		rv['operations'][0]['hrtimeStarted'] = process.hrtime();
	}
